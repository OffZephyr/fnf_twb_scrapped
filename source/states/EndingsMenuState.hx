...
class EndingsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var end:FlxSprite = new FlxSprite(775, 2275).loadGraphic(Paths.image('end'));
		end.scrollFactor.set(1, 3);
		end.setGraphicSize(Std.int(end.width * 0.65));
		end.updateHitbox();
		//end.screenCenter(X);
	...
	function changeItem(huh:Int = 0)
	{
		...
		menuItems.forEach(function(spr:FlxSprite)
		{
			...
			if (curSelected == 1)
				{
					camFollow.setPosition(1200, 1100);
					spr.centerOffsets();
				}
