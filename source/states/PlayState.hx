...
class PlayState extends MusicBeatState
{
	...
	public static var isJokeStage(get, never):Bool;
	public static var isFireStage(get, never):Bool;
	public static var isSleepStage(get, never):Bool;
	public static var isSlowlyStage(get, never):Bool;
	public static var isBorgorStage(get, never):Bool;
	public static var isClassicStage(get, never):Bool;

	@:noCompletion
	static function get_isPixelStage():Bool
		return stageUI == "pixel";
	static function get_isJokeStage():Bool
		return stageUI == "joke";
	static function get_isFireStage():Bool
		return stageUI == "fire";
	static function get_isSleepStage():Bool
		return stageUI == "sleep";
	static function get_isSlowlyStage():Bool
		return stageUI == "slowly";
	static function get_isBorgorStage():Bool
		return stageUI == "borgor";
	static function get_isClassicStage():Bool
		return stageUI == "classic";
	...
	private var healthBarBorgorNight:AttachedSprite;
	private var healthBarBorgorEvening:AttachedSprite;
	private var healthBarBorgor:AttachedSprite;
	private var healthBarJoke:AttachedSprite;
	private var healthBarSleep:AttachedSprite;
	private var healthBarFire:AttachedSprite;
	private var healthBarDeco:AttachedSprite;
	...
	override public function create()
	{
		...
		healthBarDeco = new AttachedSprite('healthdeco');
		healthBarDeco.y = objects.HealthBar.bgy - 190;
		healthBarDeco.screenCenter(X);
		healthBarDeco.scrollFactor.set();
		healthBarDeco.visible = !ClientPrefs.data.hideHud;
		healthBarDeco.alpha = ClientPrefs.data.healthBarDecoAlpha;
		add(healthBarDeco);
		
		healthBarFire = new AttachedSprite('healthdecofire');
		healthBarFire.y = healthBarDeco.y;
		healthBarFire.screenCenter(X);
		healthBarFire.scrollFactor.set();
		healthBarFire.visible = !ClientPrefs.data.hideHud;
		healthBarFire.alpha = ClientPrefs.data.healthBarFireAlpha;
		add(healthBarFire);

		healthBarSleep = new AttachedSprite('healthdecosleep');
		healthBarSleep.y = healthBarDeco.y;
		healthBarSleep.screenCenter(X);
		healthBarSleep.scrollFactor.set();
		healthBarSleep.visible = !ClientPrefs.data.hideHud;
		healthBarSleep.alpha = ClientPrefs.data.healthBarSleepAlpha;
		add(healthBarSleep);

		healthBarJoke = new AttachedSprite('healthdecojoke');
		healthBarJoke.y = healthBarDeco.y;
		healthBarJoke.screenCenter(X);
		healthBarJoke.scrollFactor.set();
		healthBarJoke.visible = !ClientPrefs.data.hideHud;
		healthBarJoke.alpha = ClientPrefs.data.healthBarJokeAlpha;
		add(healthBarJoke);

		healthBarBorgor = new AttachedSprite('healthdecoborgor');
		healthBarBorgor.y = healthBarDeco.y;
		healthBarBorgor.screenCenter(X);
		healthBarBorgor.scrollFactor.set();
		healthBarBorgor.visible = !ClientPrefs.data.hideHud;
		healthBarBorgor.alpha = ClientPrefs.data.healthBarBorgorAlpha;
		add(healthBarBorgor);

		healthBarBorgorEvening = new AttachedSprite('healthdecoborgorevening');
		healthBarBorgorEvening.y = healthBarDeco.y;
		healthBarBorgorEvening.screenCenter(X);
		healthBarBorgorEvening.scrollFactor.set();
		healthBarBorgorEvening.visible = !ClientPrefs.data.hideHud;
		healthBarBorgorEvening.alpha = ClientPrefs.data.healthBarBorgorEveningAlpha;
		add(healthBarBorgorEvening);

		healthBarBorgorNight = new AttachedSprite('healthdecoborgornight');
		healthBarBorgorNight.y = healthBarDeco.y;
		healthBarBorgorNight.screenCenter(X);
		healthBarBorgorNight.scrollFactor.set();
		healthBarBorgorNight.visible = !ClientPrefs.data.hideHud;
		healthBarBorgorNight.alpha = ClientPrefs.data.healthBarBorgorNightAlpha;
		add(healthBarBorgorNight);
		...
		healthBarDeco.cameras = [camHUD];
		healthBarFire.cameras = [camHUD];
		healthBarSleep.cameras = [camHUD];
		healthBarJoke.cameras = [camHUD];
		healthBarBorgor.cameras = [camHUD];
		healthBarBorgorEvening.cameras = [camHUD];
		healthBarBorgorNight.cameras = [camHUD];
	...
	{
		#if desktop
		if (health > 0 && !paused) DiscordClient.changePresence(detailsPausedText, SONG.song, iconP2.getCharacter());
		#end
	...
	public function endSong()
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				currentSong = Paths.formatToSongPath(SONG.song);
				switch (currentSong){
					case "hanging-on":
						PlayState.SONG = backend.Song.loadFromJson('puppetry', 'puppetry');
					...
					case "a-beautilful-end":
						MusicBeatState.switchState(new CreditsState());
						ClientPrefs.data.galleryLock = false;
						FlxG.save.data.galleryLock = false;
					...
					case "borgor":
						MusicBeatState.switchState(new BorgorOneState());
						FlxG.sound.playMusic(Paths.music('borgorEND1'));
		
						cancelMusicFadeTween();
						if(FlxTransitionableState.skipNextTransIn) {
							CustomFadeTransition.nextCamera = null;
							}
					case "held-by-a-thread":
	...
	private function cachePopUpScore()
	{
		...
		if (stageUI != "normal")
		{
			...
			if (PlayState.isSlowlyStage) uiSuffix = '-slowly';
			if (PlayState.isJokeStage) uiSuffix = '-joke';
			if (PlayState.isFireStage) uiSuffix = '-fire';
			if (PlayState.isSleepStage) uiSuffix = '-sleep';
			if (PlayState.isBorgorStage) uiSuffix = '-borgor';
			if (PlayState.isClassicStage) uiSuffix = 'classic';
		}
