...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bg = new FlxSprite(0).loadGraphic(Paths.image('menuBGunderlay'));
		...
		modlogo = new FlxSprite(0).loadGraphic(Paths.image('modLogo'));
		modlogo.scrollFactor.set(0, 0);
		modlogo.setGraphicSize(Std.int(modlogo.width * 1));
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxTween.tween(modlogo, {"scale.x": 1.25,"scale.y": 1.25}, 1.5, {
